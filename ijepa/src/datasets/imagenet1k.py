...
def make_cifar10(
    ...
    # Load ImageNet from Hugging Face datasets
    dataset = load_dataset("cifar10", split={'train': 'train[:80%]', 'validation': 'train[80%:]'})
    
    # Apply any filtering with subset_file if necessary
    if subset_file:
        with open(subset_file, 'r') as f:
            subset_ids = set(f.read().splitlines())
